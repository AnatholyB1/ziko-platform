  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#F7F6F3' }}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 20, paddingTop: 8, paddingBottom: 12, gap: 12 }}>
        ...
        <Text style={{ fontSize: 22, fontWeight: '700', color: '#1C1A17', flex: 1 }}>Amis</Text>
      </View>
        ...
        {TABS.map((t) => {
          ...
          return (
            <TouchableOpacity key={t.key} onPress={() => setTab(t.key)}
              ...
              <Ionicons name={t.icon as any} size={14} color={active ? '#FFFFFF' : '#7A7670'} />
              <Text style={{ fontSize: 13, fontWeight: '600', color: active ? '#FFFFFF' : '#1C1A17' }}>{t.label}</Text>
            </TouchableOpacity>
          );
        })}
      </ScrollView>
        ...
        {tab === 'search' && (
          <>
            <View style={{
              ...
              backgroundColor: '#FFFFFF', borderRadius: 14, borderWidth: 1, borderColor: '#E2E0DA',
              ...
              <TextInput
                ...
                style={{ flex: 1, fontSize: 15, color: '#1C1A17' }}
              />
            </View>
            <TouchableOpacity onPress={doSearch}
              style={{ backgroundColor: '#FF5C1A', borderRadius: 12, paddingVertical: 12, alignItems: 'center' }}>
            ...
            {results.map((u) => (
              <View key={u.id} style={{
                ...
                <Avatar name={u.name} />
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: 15, fontWeight: '600', color: '#1C1A17' }}>{u.name || 'Utilisateur'}</Text>
                  <Text style={{ fontSize: 12, color: '#7A7670' }}>{u.goal || ''}</Text>
                </View>
            ...
            {results.length === 0 && query.length > 0 && !searching && (
              <Text style={{ textAlign: 'center', color: '#7A7670', marginTop: 20 }}>Aucun résultat</Text>
            )}
          </>
        )}
          ...
          <>
            {pendingRequests.length === 0 && (
              <View style={{ alignItems: 'center', paddingVertical: 40 }}>
                ...
                <Text style={{ color: '#7A7670', fontSize: 14, marginTop: 8 }}>Aucune demande en attente</Text>
              </View>
            )}
            {pendingRequests.map((req) => (
              <View key={req.id} style={{
                ...
                <Avatar name="?" />
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: 14, fontWeight: '600', color: '#1C1A17' }}>Demande d'ami</Text>
                ...
                <TouchableOpacity onPress={() => handleDecline(req.id)}
                  style={{ backgroundColor: '#E2E0DA', borderRadius: 10, paddingHorizontal: 12, paddingVertical: 8 }}>
          ...
          <>
            {friends.length === 0 && (
              <View style={{ alignItems: 'center', paddingVertical: 40 }}>
                ...
                <Text style={{ color: '#7A7670', fontSize: 14, marginTop: 8 }}>Pas encore d'amis</Text>
            ...
            {friends.map((f) => (
              <View key={f.id} style={{
                ...
                <Avatar name={f.name} />
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: 15, fontWeight: '600', color: '#1C1A17' }}>{f.name || 'Ami'}</Text>
                  <Text style={{ fontSize: 12, color: '#7A7670' }}>{f.goal || ''}</Text>
                </View>
                <View style={{ flexDirection: 'row', gap: 6 }}>
                  ...
                  <TouchableOpacity onPress={() => router.push(`/(app)/(plugins)/community/compare?friendId=${f.id}` as any)}
                    style={{ width: 36, height: 36, borderRadius: 10, backgroundColor: '#FF5C1A18', alignItems: 'center', justifyContent: 'center' }}>
                    <Ionicons name="stats-chart" size={16} color="#FF5C1A" />
                  </TouchableOpacity>
                </View>
              </View>
            ))}
