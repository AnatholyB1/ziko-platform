...
function StatusBadge({ status }: { status: string }) {
  ...
  const map: Record<string, { label: string; bg: string; color: string }> = {
    ...
    completed: { label: 'Terminé', bg: '#7A767018', color: theme.muted },
    cancelled: { label: 'Annulé', bg: theme.border, color: theme.muted },
  };
  ...
  return (
    <TouchableOpacity
      ...
      style={{
        ...
        borderWidth: 1, borderColor: challenge.status === 'active' ? theme.primary + '33' : theme.border,
      }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={{ fontSize: 16, fontWeight: '700', color: theme.text }}>{challenge.title}</Text>
      ...
      {challenge.description && (
        <Text numberOfLines={2} style={{ fontSize: 13, color: theme.muted, marginTop: 10, lineHeight: 18 }}>
          {challenge.description}
        </Text>
      )}
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.background }}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20, paddingTop: 8, paddingBottom: 12 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
          ...
          <Text style={{ fontSize: 22, fontWeight: '700', color: theme.text }}>Défis</Text>
        </View>
      ...
      >
        {challenges.length === 0 && (
          <View style={{ alignItems: 'center', paddingVertical: 60 }}>
            ...
            <Text style={{ color: theme.muted, fontSize: 15, marginTop: 12 }}>Aucun défi</Text>
        ...
        {active.length > 0 && (
          <View style={{ gap: 10 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 4 }}>
              ...
              <Text style={{ fontSize: 16, fontWeight: '700', color: theme.text }}>En cours ({active.length})</Text>
            </View>
        ...
        {past.length > 0 && (
          <View style={{ gap: 10, marginTop: 8 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 4 }}>
              ...
              <Text style={{ fontSize: 16, fontWeight: '700', color: theme.muted }}>Terminés ({past.length})</Text>
            </View>
