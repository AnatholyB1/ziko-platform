  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#0F0F14' }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', padding: 20, paddingBottom: 12 }}>
        ...
        <Text style={{ flex: 1, fontSize: 22, fontWeight: '800', color: '#F0F0F5' }} numberOfLines={1}>
      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingTop: 0, paddingBottom: 100 }}>
        {program?.description && (
          <Text style={{ color: '#8888A8', fontSize: 14, marginBottom: 20 }}>{program.description}</Text>
        )}

        {workouts.length === 0 && (
          <View style={{ alignItems: 'center', marginTop: 60 }}>
            <Text style={{ fontSize: 40 }}>📋</Text>
            <Text style={{ color: '#8888A8', fontSize: 15, marginTop: 12 }}>No workouts in this program yet</Text>
          </View>
        )}

        {workouts.map((workout) => (
          <View key={workout.id} style={{ backgroundColor: '#1A1A24', borderRadius: 16, padding: 16, marginBottom: 12, borderWidth: 1, borderColor: '#2E2E40' }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              <View>
                <Text style={{ color: '#6C63FF', fontSize: 11, fontWeight: '600' }}>
                  {workout.day_of_week ? DAY_NAMES[workout.day_of_week] : 'Any day'}
                </Text>
                <Text style={{ color: '#F0F0F5', fontWeight: '700', fontSize: 16, marginTop: 2 }}>{workout.name}</Text>
              </View>
            ...
            {(workout.program_exercises ?? []).map((pe: any) => (
              <View key={pe.id} style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                ...
                <Text style={{ color: '#F0F0F5', fontSize: 13, flex: 1 }}>{pe.exercises?.name}</Text>
                <Text style={{ color: '#8888A8', fontSize: 12 }}>{pe.sets}×{pe.reps}</Text>
              </View>
            ))}
          </View>
        ))}
