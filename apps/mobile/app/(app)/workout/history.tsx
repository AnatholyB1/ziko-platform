  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#0F0F14' }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', padding: 20, paddingBottom: 12 }}>
        ...
        <Text style={{ flex: 1, fontSize: 26, fontWeight: '800', color: '#F0F0F5' }}>History</Text>
      </View>
      ...
      >
        ...
        <View style={{ flexDirection: 'row', gap: 10, marginBottom: 28 }}>
          <View style={{ flex: 1, backgroundColor: '#1A1A24', borderRadius: 16, padding: 16, borderWidth: 1, borderColor: '#2E2E40' }}>
            <Text style={{ color: '#6C63FF', fontWeight: '700', fontSize: 22 }}>{sessions.length}</Text>
            <Text style={{ color: '#8888A8', fontSize: 12, marginTop: 2 }}>Workouts (90d)</Text>
          </View>
          <View style={{ flex: 1, backgroundColor: '#1A1A24', borderRadius: 16, padding: 16, borderWidth: 1, borderColor: '#2E2E40' }}>
            <Text style={{ color: '#FF9800', fontWeight: '700', fontSize: 22 }}>
              {(totalVolume / 1000).toFixed(1)}t
            </Text>
            <Text style={{ color: '#8888A8', fontSize: 12, marginTop: 2 }}>Total volume</Text>
          </View>
          <View style={{ flex: 1, backgroundColor: '#1A1A24', borderRadius: 16, padding: 16, borderWidth: 1, borderColor: '#2E2E40' }}>
            <Text style={{ color: '#4CAF50', fontWeight: '700', fontSize: 22 }}>
              {Math.round(avgDuration)}m
            </Text>
            <Text style={{ color: '#8888A8', fontSize: 12, marginTop: 2 }}>Avg duration</Text>
          </View>
        </View>

        <Text style={{ color: '#F0F0F5', fontWeight: '700', fontSize: 16, marginBottom: 12 }}>All Sessions</Text>

        {sessions.length === 0 && (
          <View style={{ alignItems: 'center', marginTop: 48 }}>
            <Text style={{ fontSize: 40 }}>📊</Text>
            <Text style={{ color: '#F0F0F5', fontSize: 16, fontWeight: '600', marginTop: 12 }}>No sessions yet</Text>
            <Text style={{ color: '#8888A8', marginTop: 8 }}>Complete your first workout to see history</Text>
          </View>
        )}
        ...
        {sessions.map((session) => {
          const duration = session.ended_at
            ...
            : null;
          return (
            <View key={session.id} style={{ backgroundColor: '#1A1A24', borderRadius: 14, padding: 16, marginBottom: 8, borderWidth: 1, borderColor: '#2E2E40' }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <View style={{ flex: 1 }}>
                  <Text style={{ color: '#F0F0F5', fontWeight: '600', fontSize: 15 }}>{session.name ?? 'Workout'}</Text>
              ...
              {duration != null && (
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 8 }}>
                  <Ionicons name="time-outline" size={12} color="#8888A8" />
                  <Text style={{ color: '#8888A8', fontSize: 11 }}>{duration} min</Text>
                </View>
              )}
            </View>
          );
