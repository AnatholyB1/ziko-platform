  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#0F0F14' }}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', padding: 20, paddingBottom: 12 }}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={{ color: '#F0F0F5', fontWeight: '700', fontSize: 18 }}>Active Session</Text>
          <Text style={{ color: '#6C63FF', fontSize: 14, fontWeight: '600' }}>{formatTime(elapsed)}</Text>
        </View>
      ...
      {restTimer !== null && restTimer > 0 && (
        <View style={{ marginHorizontal: 20, backgroundColor: '#6C63FF22', borderRadius: 12, padding: 14, flexDirection: 'row', alignItems: 'center', marginBottom: 12 }}>
          <Ionicons name="timer" size={18} color="#6C63FF" style={{ marginRight: 8 }} />
          <Text style={{ color: '#6C63FF', fontWeight: '600', flex: 1 }}>Rest: {formatTime(restTimer)}</Text>
          <TouchableOpacity onPress={stopRestTimer}>
            <Ionicons name="close-circle" size={20} color="#6C63FF" />
          </TouchableOpacity>
        </View>
      )}
      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingTop: 0, paddingBottom: 120 }}>
        {sessionExercises.length === 0 && (
          <View style={{ alignItems: 'center', marginTop: 48 }}>
            ...
            <Text style={{ color: '#F0F0F5', fontSize: 16, fontWeight: '600', marginTop: 12 }}>Add your first exercise</Text>
            <Text style={{ color: '#8888A8', marginTop: 8, textAlign: 'center' }}>Tap the button below to pick exercises</Text>
          </View>
        )}

        {sessionExercises.map((ex, exIdx) => (
          <View key={`${ex.id}-${exIdx}`} style={{ backgroundColor: '#1A1A24', borderRadius: 16, padding: 16, marginBottom: 12, borderWidth: 1, borderColor: '#2E2E40' }}>
            <Text style={{ color: '#F0F0F5', fontWeight: '700', fontSize: 15, marginBottom: 12 }}>{ex.name}</Text>

            {/* Set headers */}
            <View style={{ flexDirection: 'row', marginBottom: 8 }}>
              <Text style={{ color: '#8888A8', fontSize: 12, width: 30 }}>Set</Text>
              <Text style={{ color: '#8888A8', fontSize: 12, flex: 1, textAlign: 'center' }}>Reps</Text>
              <Text style={{ color: '#8888A8', fontSize: 12, flex: 1, textAlign: 'center' }}>Weight (kg)</Text>
              <View style={{ width: 40 }} />
            </View>

            {ex.sets.map((set, setIdx) => (
              <View key={setIdx} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8, gap: 8 }}>
                <Text style={{ color: '#8888A8', fontSize: 14, width: 22 }}>{setIdx + 1}</Text>
                <TextInput
                  value={set.reps}
                  ...
                  style={{ flex: 1, backgroundColor: '#252535', borderRadius: 8, padding: 10, color: '#F0F0F5', textAlign: 'center' }}
                />
                <TextInput
                  value={set.weight}
            ...
            <TouchableOpacity onPress={() => addSetToExercise(exIdx)} style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 8 }}>
              ...
              <Text style={{ color: '#6C63FF', fontSize: 13 }}>Add Set</Text>
            </TouchableOpacity>
          </View>
        ))}
        ...
        <TouchableOpacity
          ...
          style={{ borderWidth: 1.5, borderColor: '#6C63FF', borderStyle: 'dashed', borderRadius: 16, padding: 16, alignItems: 'center', flexDirection: 'row', justifyContent: 'center', gap: 8 }}
        >
          <Ionicons name="add" size={20} color="#6C63FF" />
          <Text style={{ color: '#6C63FF', fontWeight: '600', fontSize: 15 }}>Add Exercise</Text>
        </TouchableOpacity>
      </ScrollView>
      ...
      <Modal visible={showExercises} animationType="slide" presentationStyle="pageSheet">
        <View style={{ flex: 1, backgroundColor: '#0F0F14', padding: 24 }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <Text style={{ color: '#F0F0F5', fontSize: 20, fontWeight: '700' }}>Choose Exercise</Text>
          ...
          <ScrollView>
            {filteredExercises.map((ex) => (
              <TouchableOpacity key={ex.id} onPress={() => addExerciseToSession(ex)}
                style={{ backgroundColor: '#1A1A24', borderRadius: 12, padding: 14, marginBottom: 8, borderWidth: 1, borderColor: '#2E2E40', flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                <View style={{ width: 36, height: 36, borderRadius: 8, backgroundColor: '#6C63FF22', alignItems: 'center', justifyContent: 'center' }}>
                  <Ionicons name="barbell-outline" size={16} color="#6C63FF" />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={{ color: '#F0F0F5', fontWeight: '500', fontSize: 14 }}>{ex.name}</Text>
                  <Text style={{ color: '#8888A8', fontSize: 11, marginTop: 2 }}>{ex.muscle_groups.join(', ')}</Text>
                </View>
              </TouchableOpacity>
            ))}
          </ScrollView>
