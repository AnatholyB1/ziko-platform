...
export class DymatizeScraper implements BrandScraper {
  brandSlug = 'dymatize';
  brandName = 'Dymatize';

  async scrape(): Promise<ScraperResult> {
    const { products: rawProducts, errors } = await fetchShopifyProducts('https://www.dymatize.com');
    ...
    for (const product of rawProducts) {
      ...
      scraped.push({
        ...
        sourceUrl: `https://www.dymatize.com/products/${product.handle}`,
        price,
        currency: 'USD',
