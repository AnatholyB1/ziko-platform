...
export class BSNScraper implements BrandScraper {
  brandSlug = 'bsn';
  brandName = 'BSN';

  async scrape(): Promise<ScraperResult> {
    const { products: rawProducts, errors } = await fetchShopifyProducts('https://www.bsn.com');
    ...
    for (const product of rawProducts) {
      ...
      scraped.push({
        ...
        sourceUrl: `https://www.bsn.com/products/${product.handle}`,
        price,
        currency: 'USD',
