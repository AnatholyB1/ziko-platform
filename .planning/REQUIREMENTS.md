...
- [ ] **CLIENT-01**: A coach can see a paginated list of all linked clients on `/coach/clients` with name, photo, last-active timestamp, and quick filters.
...
- [ ] **CLIENT-05**: A coach can attach custom text tags to a client (e.g. "Hyrox prep", "recovery phase") via a `coach_client_tags` table; tags are coach-private (never visible to client).
- [ ] **CLIENT-06**: A coach can write private notes per client in a dedicated editor (`coach_client_notes` table); notes are coach-private and version-tracked by `updated_at`.
...
- [ ] **CLIENT-08**: A coach can revoke a coach↔client link from their dashboard with a 2-step confirmation; client retains all data but coach loses read access.
