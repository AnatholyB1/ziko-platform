...
decisions:
  ...
  - Task 2 (supabase db push) was applied via Supabase MCP directly — both columns confirmed present
metrics:
  duration: "~10 minutes"
...
Both tasks are complete. The migration was applied to the live Supabase database via MCP (not `supabase db push`). The new columns exist on `notification_preferences`:
